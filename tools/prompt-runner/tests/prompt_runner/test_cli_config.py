...
def test_cli_variant_populates_config(tmp_path: Path, monkeypatch):
    from prompt_runner import __main__ as m

    pfile = tmp_path / "pr.md"
    pfile.write_text(
        "## Prompt 1: Audit [VARIANTS]\n\n"
        "### Variant A: One\n\n"
        "#### Generation Prompt\n\n"
        "gen a\n\n"
        "### Variant B: Two\n\n"
    ...
    assert rc == 0
    assert captured["config"].only == 1
    assert captured["config"].variant == "B"


def test_cli_variant_requires_only(tmp_path: Path, capsys):
    from prompt_runner import __main__ as m

    pfile = _prompt_file(tmp_path / "pr.md")

    rc = m.main(["run", str(pfile), "--dry-run", "--variant", "B"])
    assert rc == 2
    captured = capsys.readouterr()
    assert "R-INVALID-FLAGS" in captured.err


def test_cli_variant_requires_fork_prompt(tmp_path: Path, capsys):
    from prompt_runner import __main__ as m

    pfile = _prompt_file(tmp_path / "pr.md")

    rc = m.main(["run", str(pfile), "--dry-run", "--only", "1", "--variant", "B"])
    assert rc == 2
    captured = capsys.readouterr()
    assert "R-INVALID-FLAGS" in captured.err
...
def test_cli_debug_defaults_to_zero_without_flag(tmp_path: Path, monkeypatch):
    ...
    assert rc == 0
    assert captured["config"].debug == 0


def test_cli_debug_flag_defaults_to_three(tmp_path: Path, monkeypatch):
    from prompt_runner import __main__ as m
    ...
    rc = m.main(["run", str(pfile), "--dry-run", "--debug"])

    assert rc == 0
    assert captured["config"].debug == 3


def test_cli_debug_flag_accepts_explicit_depth(tmp_path: Path, monkeypatch):
    from prompt_runner import __main__ as m
    ...
    rc = m.main(["run", str(pfile), "--dry-run", "--debug", "5"])

    assert rc == 0
    assert captured["config"].debug == 5
