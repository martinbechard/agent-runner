...
| Accepted decisions | Dev Architect reconciliation packet accepted for `/root/report_app_architecture` | Component split, full Workspace semantics, generic Rust result transport, cryptographic operation IDs, structured diagnostics, native registries, lifecycle results, MCP surface, and implementation sequence |
| Backlog requirements | [Modularization backlog item](../../feature-backlog/modularize-report-tool-for-concurrent-maintenance.md) | Existing Python decomposition pressure |
...
| Current source evidence | `tools/report/scripts/run-timeline.py`, `tools/report/src/agent_report/`, `tools/report/rust/`, `tools/report/desktop/` | Compatibility and reuse evidence only |
...
Recheck this HLD when FR-001 operations, ARC-001 constraints, planned paths, report schemas, discovery protocol, privacy rules, pricing, formatter configuration, or compatibility policy changes. The latest source review is 2026-08-12.
...
| FR-001 FR-03; INTENDED_BEHAVIOR | The client gets the summary, changes views, pages agents/turns/events, requests time, sequence, and coordination data, and opens redacted detail lazily. | Application Service, Event Repository, Workspace; CR-10, CR-13; OP-18 through OP-25 | `SnapshotReady -> Querying -> SnapshotReady`; queries never replace the snapshot. | Invalid page size returns `REPORT_INVALID_REQUEST`; stale cursor or filter mismatch returns cursor conflict; missing event returns `REPORT_EVENT_NOT_FOUND`; oversized time range returns a coarser actual resolution. | DEFINED; service paging, query, detail, DTO, view, and accessibility tests |
...
| OP-22 | `query_time_range` snapshot form; MCP `query_snapshot_time_range` | Snapshot holder | Snapshot; half-open visible range; measure; `agent|event_kind|work_item` grouping; 1, 5, 15, 30, or 60 requested minutes; maximum 1-200 rows | Exact-revision grouped heatmap with at most 2,000 total cells, actual resolution, ordered rows, omission count, per-row scale basis, color semantic, labels, and provenance | Query limits; grouping, coarsening, row-scale, color, and MCP parity tests | DEFINED; Application Service |
...
| Snapshot binding | Transient and persisted identity | ARC-09 | Application Service owns opaque ID and binding | Bind source revisions, scope, parser/pricing/formatter versions, observation time, live/sealed state. |
...
| Grouped heatmap aggregation | Derived DTO | FR-001 FR-03 | Application Service derives it from normalized events | Recompute per query; maximum 2,000 total cells across all rows and report actual resolution. |
| Worker operation envelope | Process contract | ARC-03, ARC-14 | Worker and Supervisor exchange versioned JSONL | Each non-handshake operation ID contains 96 cryptographically secure random bits encoded as `op_` plus 24 lowercase hexadecimal characters. Progress is bounded. One terminal result follows. Python validates operation-specific service results; Rust transports the result as a generic bounded JSON object. |
...
flowchart LR
  ...
  Snapshot --> Aggregate[Derived aggregates]
...
|---|---|---|
| Application Service | Own exact operations, scope, snapshots, limits, structured errors, and semantic parity. | Planned: `docs/design/components/CD-002-agent-report-application-service.md` |
| Event Repository | Own normalized cache, migrations, invalidation, snapshot persistence, and atomic revision publication. | Planned: `docs/design/components/CD-003-agent-report-normalized-event-cache.md` |
| Python Worker | Adapt versioned stdio operations to one Application Service instance. | Planned: `docs/design/components/CD-004-agent-report-worker-protocol.md` |
...
| Dynamic Workspace | Render bounded summary, coordination, heatmap, timeline, sequence, agent, turn, tool, model, context, inference, runtime, wait, work-item, claim, detail, provenance, and diagnostic views. | Planned: `docs/design/components/CD-005-agent-report-dynamic-workspace.md` |
...
| MCP Adapter | Retain existing MCP tools and add shared operations without Tauri. | HLD-003 and existing MCP source are sufficient until scope changes. |
...
| Normalization And Query Core | Reuse current Python normalization, metrics, privacy, coordination, and rendering semantics. | [CD-001](../components/CD-001-codex-rollout-metrics.md) and later focused refactoring designs |
...
| HLP-06 | Existing MCP, CLI, discovery, and normalization paths | ARC-02, ARC-04, and ARC-13 retain independent MCP/CLI entry points, Rust-only discovery, and current Python semantics. | Reusing existing files is the smallest viable approach. It avoids parallel adapters, a second discovery engine, and an unnecessary normalization rewrite. | Dev Architect; accepted packet for `/root/report_app_architecture` |
...
No separate migration folder is planned. `event_cache.py` owns schema resources until CD-003 selects a different accepted location. CD-003 owns internal table names and migration SQL.
...
| BE-08 | Application Service to Normalization Core | Accepted sources and snapshot configuration | CR-08 | TB-08 |
| BE-09 | Normalization Core to Event Repository | Sanitized normalized revision publication | CR-09 | TB-09 |
| BE-10 | Application Service to Event Repository | Snapshot, query, invalidation, and maintenance contract | CR-10 | TB-10 |
...
| BE-13 | Application Service to Workspace | Bounded sanitized response DTO | CR-13 | TB-13 |
...
| TB-08 / Normalization | Application Service under OS user | Coordinator; transcript-derived evidence | Accepted snapshot scope and versions | Service owns binding; Core owns semantics; tenancy N/A; revision/privacy filters | Exact revisions/versions; mismatch is conflict | Privacy filter before repository, DTO, inline, or export; preserve evidence labels | Service validates binding; Core validates records; Service owns operation state | Bounded worker task; cancellation/parse error before publication. See CR-08. |
...
| TB-10 / Cache query and maintenance | Snapshot holder or Tauri maintainer under OS user | Query/maintenance roles; derived snapshots and disk use | Snapshot authorizes reads; Tauri capability authorizes event-cache purge | Repository owns event cache only; tenancy N/A; snapshot or maintenance predicate filters | Snapshot/cursor/predicate; not-found, stale, invalid, or newer schema explicit | No event-cache path to MCP/webview; never purge source/discovery | Service and Repository validate; Repository owns revision/maintenance state | Consistent read or atomic write/purge; cancellation preserves commit. See CR-10. |
...
| TB-13 / Query response | Snapshot holder through validated surface | Viewer; normalized summary/list/aggregate/detail | Valid snapshot and operation selector | Service owns DTO/snapshot; Workspace owns render; tenancy N/A; filter/cursor/event bounds | Exact snapshot/cursor/filter/event; mismatch is validation/conflict/not-found | Bounded sanitized DTO; raw record, cache path, and filesystem authority absent | Service and Tauri validate; Service owns query state; Workspace owns view state | Async result; cancel/error before view data replacement. See CR-13. |
...
Only one mutating operation for a snapshot revision runs at a time. Read queries observe one published revision. Tauri first sends a cancellation token. If the grace period expires, Tauri terminates its worker tree. Other adapters own equivalent process cleanup without depending on Tauri.
...
| Grouped heatmap result | Application Service | Snapshot and revision; measure; grouping; exact range; requested and actual resolution; applied row maximum; omitted-row count; deterministic row order; total cell count; rows with independent scale basis, color semantic, cells, labels, evidence; provenance | Heatmap and MCP `query_snapshot_time_range` consumers |
...
flowchart LR
  ...
  Service --> Pages[Pages and aggregates]
...
|---|---|---|---|---|---|---|---|---|---|
| CR-01 / BE-01 | Workspace to Tauri Supervisor | Local operator; authenticated only by the OS-user Tauri process | Role: operator. Tauri capabilities authorize commands. Tauri owns native resources; Workspace owns UI state. Tenancy: N/A. Filter: selected roots/snapshot and bounded DTO fields. | Exact command, snapshot, scope, cursor, or event selector. Unknown command, stale selector, or invalid DTO is rejected before native work. | Request DTO only; no raw rollout, cache path, or unrestricted filesystem path. Sensitive fields are absent or redacted. | Tauri command boundary | Workspace owns request state; Application Service owns report state. UI moves pending to result/error without changing snapshot on validation failure. | Async native command. Cancellation uses operation ID. Validation error precedes side effects; operation error follows terminal worker/service result. | AGREED |
| CR-02 / BE-02 | Tauri Supervisor to Python Worker | Background worker spawned by the same OS-user Tauri host | Role: service worker. Supervisor authorizes only its spawned process. Supervisor owns process; Worker owns protocol handling. Tenancy: N/A. Filter: protocol operation and native-configured roots. | Protocol version, operation ID, operation name, optional snapshot. Version/ID mismatch fails before dispatch. Duplicate terminal record is protocol failure. | Versioned JSONL only; stdout has protocol records, stderr/logs are bounded; no intentional transcript body. | Supervisor validates envelope framing/version; Worker validates request semantics before service call. | Supervisor owns process lifecycle; Worker owns operation pending/progress/terminal transition. | Async stdio. Cooperative cancel first; forced process-tree termination after grace. EOF/protocol failure is terminal and preserves last coherent snapshot. | AGREED |
| CR-03 / BE-03 | Python Worker to Application Service | Background worker; same process and OS identity | Role: service adapter. Service authorizes configured operations. Worker owns transport; Service owns semantics. Tenancy: N/A. Filter: operation arguments and bound scope. | Exact operation and snapshot selector. Unknown operation, bad version, or stale snapshot returns structured error. | In-process request/result shapes; disclosure follows operation DTO contract and privacy filter. | Application Service | Service owns operation and snapshot transition; Worker mirrors one terminal state. | In-process synchronous call within async worker operation. Cancellation token is checked during work; validation precedes mutation; terminal error follows rollback/no publication. | AGREED |
| CR-04 / BE-04 | MCP Adapter to Application Service or Classic Renderer | MCP client over configured stdio; server runs under local OS user | Role: MCP caller. Server configuration authorizes session/output roots. Adapter owns MCP surface; Service owns snapshot state; Classic Renderer owns retained report generation. Tenancy: N/A. Filters: configured roots and exact selectors. | Exact retained or snapshot-tool selector. Multiple time/name matches return bounded ambiguity; invalid selector/range/format/version returns validation; stale cursor/snapshot returns conflict. | Structured MCP response; complete inline data or oversize metadata; never cache path, unrestricted source path, truncated inline, or unredacted event. | MCP Adapter validates server/surface config; Application Service validates snapshot semantics; Classic Renderer retains generation semantics. | Service instance owns snapshots; retained `generate_report` ends at its current terminal response. | In-process call inside async MCP tool. Cancellation is structured. Discovery/generation/write errors occur before success; written-file warnings remain explicit. | AGREED |
...
| CR-08 / BE-08 | Application Service to Normalization Core | Service actor under local OS user | Role: normalization coordinator. Snapshot scope authorizes exact source revisions. Service owns binding; Core owns normalization semantics. Tenancy: N/A. Filter: accepted source set, parser/pricing/formatter versions, privacy rules. | Exact revisions and versions. Changed/missing source or binding mismatch returns conflict; parse/unsupported record affects the documented boundary. | Source records enter Core; only sanitized normalized values leave. Ciphertext stays opaque; epistemic labels and pricing provenance remain. | Application Service validates binding; Core validates source records and semantic inputs. | Service owns operation/snapshot state; Core owns transient parse/query state. | Bounded worker task. Cancellation is checked during parse/query. Any error precedes repository/export publication. | AGREED |
...
| CR-10 / BE-10 | Application Service to Event Repository | Snapshot holder through validated surface, or Tauri maintenance actor; local OS identity | Roles: query caller or cache maintainer. Snapshot authorizes reads; Tauri maintenance capability authorizes derived-cache purge. Repository owns cache. Tenancy: N/A. Filters: snapshot scope, or deterministic LRU predicate over closed unprotected snapshots when bytes exceed the configurable 5,368,709,120-byte default. | Opaque snapshot/cursor or maintenance selector. Not-found, stale binding, cursor mismatch, invalid predicate, newer schema, or protected over-quota state is explicit. Seven-day cursor retention does not override snapshot validity. | Bounded normalized rows/metadata only; cache path is not disclosed to MCP/webview; purge never reaches open/protected snapshots, source logs, or discovery cache. | Application Service validates caller/selector; Repository validates schema/binding/transaction and exact eviction eligibility. | Repository owns snapshot-revision, protection, last-use, cursor-retention, and maintenance transitions; Service owns open/query/refresh/close state. No age-expiry transition exists. | Consistent read or atomic write/purge. Cancellation preserves committed revision. Errors occur before state transition. | AGREED |
...
| CR-13 / BE-13 | Application Service to Dynamic Workspace | Snapshot holder through validated Tauri command; local OS-user app | Role: operator viewer. Snapshot authorizes bounded read. Service owns DTO; Workspace owns rendering. Tenancy: N/A. Filters: operation, view, snapshot, cursor, event. | Snapshot/cursor/filter/event selector. Mismatch returns validation, cursor conflict, snapshot conflict, or event-not-found without closing snapshot. | Bounded sanitized DTO only; no raw record, cache path, or filesystem authority. Detail disclosure is explicit and redacted. | Service validates semantic result; Tauri validates outbound DTO; Workspace validates discriminated UI contract. | Service owns snapshot/query state; Workspace owns active-view/pending/result/error and clears invalidated cursor/detail on refresh. | Async response. Cancellation returns terminal cancelled state. Error arrives before UI commits replacement view data. | AGREED |
...
| Page and heatmap limits | Application Service | Versioned defaults: page 100, maximum 500; grouped heatmap maximum 2,000 total cells | All adapters and Workspace |
...
1. Implement the accepted CD-002, CD-004, CD-005, and CD-006 source-contract corrections and freeze DEC-01 through DEC-04 in application, cache, UI, CLI, MCP, classic-renderer, and exporter tests.
2. Characterize classic CLI, MCP `generate_report`, and static baseline behavior with regression fixtures.
3. Add the Application Service and structured operations and errors without changing outputs.
4. Add scope closure and read-only preflight through the Rust discovery engine.
5. Add the Event Repository, migrations, invalidation, snapshot binding, exact quota/protection/LRU policy, seven-day cursor retention, and bounded summary.
6. Add the Python Worker, Tauri Supervisor, DTO validation, and dynamic summary.
7. Add cursor views, time aggregation, sequence, coordination, and lazy detail.
8. Add explicit refresh, cooperative cancellation, forced Tauri recovery, and last-coherent-snapshot preservation.
9. Add one shared streamlined Codex export function with offline directory, bounded summary, staging, and atomic publication for Tauri, explicit CLI choices, and MCP `export_snapshot`.
10. Pass CLI, Tauri, and MCP semantic-parity and privacy gates.
11. Pass platform packaging, offline-browser, retained MCP-tool/schema, classic-renderer, and non-Codex static-backend release gates.

```mermaid
flowchart LR
  S1[1 Freeze accepted decisions] --> S2[2 Baseline parity]
  S2 --> S3[3 Shared service]
  S3 --> S4[4 Scope and preflight]
  S4 --> S5[5 Repository and snapshot]
  S5 --> S6[6 Worker and dynamic summary]
  S6 --> S7[7 Bounded views]
  S7 --> S8[8 Refresh and recovery]
  S8 --> S9[9 Static export]
  S9 --> S10[10 Semantic and privacy parity]
  S10 --> S11[11 Packaging and release gates]
```
...
The subsystem is complete when an operator can select a bounded root, inspect a preflight, open one coherent snapshot, use every required dynamic view, refresh or cancel safely, and publish verified offline output. CLI retains its classic interactive Codex default and separate non-Codex backends. MCP retains the exact `generate_report` schema, classic bundle, tool identities, and independent runtime. Explicit streamlined operations use shared application semantics without requiring Tauri.

The implementation is observable through bounded progress and diagnostics. It is maintainable through the accepted component split and literal paths. Automated tests prove privacy, coherence, pagination, accessibility, offline operation, atomic publication, platform packaging, and cross-entry-point parity.

## Documentation Acceptance

**ACCEPTED.** This PLANNED_DEVELOPMENT HLD traces FR-01 through FR-10 and ARC-01 through ARC-16 to concrete interactions, CR boundaries, states, errors, and verification. It records DEC-01 through DEC-04, retains classic CLI and MCP generation plus non-Codex static backends, defines the shared streamlined Codex exporter and cache policy, and removes all product-decision blockers.

## Implementation Readiness

**BLOCKED.** The documentation contract is accepted. Production readiness remains blocked until source and test changes implement the synchronized CD-002, CD-004, CD-005, and CD-006 contracts, the CD-003 repository contract, MCP schema additions, native registry projections, and required platform evidence. No unresolved product or architectural choice remains.

## Verification

| Verification area | Exact evidence or planned path | Coverage |
|---|---|---|
| Application operations and errors | `tools/report/tests/test_application_service.py` | OP-16 through OP-28 and OP-30, canonical pages, grouped heatmap, full sequence/coordination/detail shapes, lifecycle results, native export fields, CR-03 through CR-13 |
| Event cache, snapshot, migration, invalidation, quota, and recovery | `tools/report/tests/test_event_cache.py` | ARC-08, ARC-09, ARC-14, DEC-02, OP-41, CR-09, CR-10 |
| Worker protocol and cooperative cancellation | `tools/report/tests/test_report_worker.py` | OP-31 through OP-34, cryptographic ID validation, operation-specific Python schema validation, generic JSON result emission, CR-02, CR-03 |
| Tauri supervision and forced recovery | `tools/report/desktop/src-tauri/tests/report_worker.rs` | BE-01, BE-02, TB-01, TB-02, generic Rust result transport, source/export registry projections |
| Dynamic UI, paging, views, navigation, accessibility | `tools/report/desktop/src/report-workspace.test.ts`, `tools/report/desktop/src/contracts.test.ts` | FR-03, FR-10, OP-03, OP-18 through OP-25, cryptographic IDs, structured diagnostics, refresh/close results, exact export counts |
...
| Current CLI and classic renderer behavior | `tools/report/tests/test_cli.py`, `tools/report/tests/test_run_timeline.py` | OP-05 through OP-12, classic omitted-mode report and sequence, and exit semantics |
| MCP retained and snapshot behavior | `tools/report/tests/test_mcp_report.py`, `tools/report/tests/test_mcp_server.py` | OP-13 through OP-15, OP-43, exact retained schemas/defaults, classic `generate_report`, exact additive snapshot-tool names, and independent startup |
...
1. Run `python -m pytest tools/report/tests` from `tools/report` in the supported environment.
2. Run TypeScript tests and `pnpm build` from `tools/report/desktop`.
