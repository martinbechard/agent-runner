...
def test_phase_0_single_phase_runs_end_to_end_with_mock_catalog(
    ...
    # The skill pack
    _write_skill(user_home / ".claude" / "skills", "ph000-requirements-extraction", "Extract reqs")
    _write_skill(user_home / ".claude" / "skills", "traceability-discipline", "Universal traceability")
    _write_skill(user_home / ".claude" / "skills", "ph000-requirements-quality-review", "QA reqs")
...
phases:
  PH-000-requirements-inventory:
    generator_baseline:
      - ph000-requirements-extraction
      - traceability-discipline
    judge_baseline:
      - ph000-requirements-quality-review
      - traceability-discipline
""",
        encoding="utf-8",
    )
    ...
    selector_reply = """\
{
  ...
  "selector_model": "test",
  "generator_skills": [
    {"id": "ph000-requirements-extraction", "source": "baseline", "rationale": "Baseline"},
    {"id": "traceability-discipline", "source": "baseline", "rationale": "Baseline"}
  ],
  "judge_skills": [
    {"id": "ph000-requirements-quality-review", "source": "baseline", "rationale": "Baseline"},
    {"id": "traceability-discipline", "source": "baseline", "rationale": "Baseline"}
  ],
...
"""
    ...
    assert "ph000-requirements-extraction" in content
    # Prelude files exist
