...
| DEC-04 | The shipped dynamic workspace is Tauri-only. Development may run the same workspace through a deterministic fixture transport or authenticated loopback adapter backed by the existing native host. | The harness binds only to loopback, uses a per-launch token and opaque native references, and is never packaged as a product interface. Static output remains `file://` compatible. |
...
|---|---|---|---|---|---|
| FR-001 FR-01; INTENDED_BEHAVIOR | The operator searches configured roots with text and inclusive local date-hour filters, receives ordered virtual rows, and selects one root. Relationship scope comes from persistent configuration. | Dynamic Workspace, Tauri host, Discovery; CR-01, CR-06, CR-07; OP-01, OP-35, OP-36, OP-44 | `Ready -> Searching -> Ready`; selection becomes the input to `Opening`. | Missing root, reversed range, discovery failure, or empty result preserves filters and permits correction or configuration. | DEFINED; `contracts.test.ts`, `catalog.rs`, `discovery.rs`, configuration and workspace tests |
| FR-001 FR-02; INTENDED_BEHAVIOR | Configuration atomically persists scan folders, independent relationship defaults, worker threads, report output folder, database folder, and database filename. `open_snapshot` resolves scope and creates or reuses a coherent revision in one cancellable operation without preflight. | Dynamic Workspace, Tauri host, Application Service, Discovery, Event Repository; CR-01, CR-03, CR-06, CR-07, CR-10; OP-17, OP-44 through OP-46 | `Ready -> Configuring -> Ready` and `Ready -> Opening -> SnapshotReady|Ready`; cancellation creates no snapshot. | Invalid settings do not partially apply. Source conflict, discovery failure, or cancellation returns no new snapshot. Database identity is immutable during active snapshot work and saving a later change never deletes the old database. | DEFINED; configuration, service, discovery, repository, and worker tests |
...
| OP-17 | `open_snapshot` | Authorized caller | Root plus independent children and collaborators booleans; desktop values come from saved configuration | Resolves authorized closure and returns opaque snapshot ID and coherent metadata; cache build or reuse; structured source-conflict/cancellation | Snapshot policy; service, repository, worker, and scope tests | DEFINED; Application Service |
...
| OP-43 | MCP snapshot-tool registration | MCP server startup | Register `open_snapshot`, `get_summary`, `list_agents`, `list_turns`, `list_events`, `query_snapshot_time_range`, `query_sequence`, `query_coordination`, `get_snapshot_event_details`, `refresh_snapshot`, `export_snapshot`, and `close_snapshot` beside retained tools | Each tool maps to the shared service and returns its structured result/error. Retained `generate_report`, `query_time_range`, and `get_event_details` schemas and defaults remain unchanged. | MCP schema snapshots, name inventory, no-export forensic flow, retained-schema tests, and snapshot-export tests | DEFINED; MCP Adapter and Application Service |
| OP-44 | Load desktop configuration | Desktop startup and local operator | No request fields | Return schema version, ordered scan-folder references, independent relationship defaults, worker threads 1 through 64, report-folder reference, database-folder reference, and `.sqlite3` database filename | Configuration schema, migration, restart, and disclosure tests | DEFINED; Tauri host owns persistence; Dynamic Workspace parses and presents |
| OP-45 | Choose configuration folder | Local operator through Tauri | Exact purpose `scan|report_output|database` and optional existing opaque reference | Return a validated opaque folder reference and display label or cancellation; do not persist | Native chooser, purpose-validation, cancellation, and path-disclosure tests | DEFINED; Tauri host |
| OP-46 | Save desktop configuration | Local operator through Tauri | Complete configuration draft returned by OP-44/45 | Validate and atomically replace the versioned configuration. Disable database identity changes during active snapshot/report work; a later switch never deletes the prior database. | Validation, atomic-save, cancellation, restart, active-work guard, and database-switch tests | DEFINED; Tauri host owns persistence/validation; Dynamic Workspace owns draft presentation |
...
The subsystem includes persistent desktop configuration, run selection integration, configured relationship scope, snapshot lifecycle, bounded queries, information and action surfaces, privacy-safe detail, explicit refresh, cancellation, recovery, static publication, compatibility adapters, navigation, diagnostics, and cache maintenance.
...
| Normalized event cache | Persisted derived data | ARC-08; DEC-02; FR-001 DEC-05 | Event Repository owns the configured `.sqlite3` database, byte accounting, last-use ordering, open/closed state, and protection state | Initial folder/name default is `~/.codex/agent-report/report-events-v1.sqlite3`; Tauri supplies the validated configured path at worker startup. Quota/LRU rules remain unchanged. |
...
| Desktop report configuration | Persisted configuration | FR-001 DEC-05/FR-02; ARC-05, ARC-07, ARC-08 | Tauri owns ordered source roots, independent relationship defaults, report output folder, database folder, and database filename | Save is atomic after native validation. Root or scope changes affect subsequent searches/opens. Database identity cannot change during active work; a later switch never deletes the prior database. |
| Authorized source roots | Configuration | FR-001 FR-01/FR-07; ARC-05, ARC-07 | Tauri settings, CLI flags, or MCP configured session roots supply an ordered set; each composition root validates existence and authority | Discovery and source readers consume only validated roots. A changed desktop root set affects the next search and snapshot open. |
...
| Workspace information families | UI surface | FR-001 UI Information And Action Contract | Dynamic Workspace exposes the required run, scope, progress, time, utilization, token, context, model, cost, agent, turn, event, coordination, sequence, provenance, diagnostic, and export information without fixing page count or layout | UX design may combine or separate information families. Every surface stays bounded and maps to the owning application-service operations. |
| Workspace interaction state | Transient UI state | FR-001 Workflows and UI Information And Action Contract | Dynamic Workspace owns active information context, snapshot ID, per-context filters/cursor, selected event, pending operation, and focus-return context in memory | State lasts for the open workspace. Refresh clears stale cursors and selected detail while preserving valid context. It is not persisted in the event cache. |
...
| Runtime event cache | Configured folder and `.sqlite3` filename; initial default `~/.codex/agent-report/report-events-v1.sqlite3` | Derived runtime data; Event Repository | Configuration, cache inspection, and recovery tests |
...
sequenceDiagram
  ...
  Operator->>UI: Select root and open report
  UI->>Host: open_snapshot with configured scope defaults
  Host->>Worker: Versioned operation
  Worker->>Service: open_snapshot
  Service->>Discovery: Discover closure and revisions
  Discovery-->>Service: Authorized identities and revisions
...
| TB-06 / Discovery request | Application Service under OS user | Discovery client; local store enumeration | Validated caller-bounded roots | Service owns scope; Discovery owns derived identities; tenancy N/A; time/text/relationship filters | Open predicate; missing root/range/protocol mismatch before snapshot | Bounded metadata only; no transcript body in result/cache | Service and Discovery validate their boundaries; Service owns snapshot-open state | Read-only async scan; cancellation/error before snapshot. See CR-06. |
...
stateDiagram-v2
  ...
  Ready --> Configuring: Open configuration
  Configuring --> Ready: Save valid settings or cancel
  Ready --> Opening: Open selected report
...
| Structured error | Application Service | Every error has `code` and safe `message`. Operation-bound errors also have `operation_id`. `recoverable` states whether the caller can retry without reopening. Selection ambiguity adds bounded `matches`. Inline oversize adds `actual_bytes`, `maximum_bytes`, `written_files`, and warnings. Source/snapshot conflict adds the current revision. Cursor conflict identifies restart-from-first-page. No error includes raw records or cache paths. | All adapters and UI. Surface adapters preserve codes and stable context while mapping presentation or CLI exit behavior. |
| Desktop configuration | Tauri host | Ordered scan folders, independent `include_children` and `include_collaborators` defaults, report output folder, database folder, and validated `.sqlite3` filename, plus a configuration schema version | Native persistence boundary. Webview receives bounded display values and opaque folder references where path disclosure is restricted. Save replaces the complete configuration atomically. |
...
flowchart LR
  ...
  Service --> Snapshot[Scope resolution and snapshot]
...
4. Add configured scope closure to snapshot opening through the Rust discovery engine.
...
flowchart LR
  ...
  S3 --> S4[4 Configuration and snapshot scope]
...
**BLOCKED.** The HLD now adopts direct snapshot opening, Tauri-owned desktop configuration, configurable normalized-database identity, and the layout-neutral information/action contract. Documentation acceptance cannot be restored until CD-002, CD-003, and CD-004 remove their preflight operations and fixed-database assumptions and their operation inventories are reconciled with OP-17 and OP-44 through OP-46.

## Implementation Readiness

**BLOCKED.** CD-002, CD-003, and CD-004 still describe the rejected preflight and fixed normalized-database path. Implementation must not proceed from the mixed contract. After those designs are reconciled, delivery still requires removal of preflight code/tests, configuration persistence and direct-open implementation, independent verification, packaged browser UX verification, platform evidence, and final review.
...
7. Complete MCP snapshot, query, detail, refresh, and close without calling a generation or export operation.
