...
| FR-001 HM-F01 through HM-F08 | INTENDED_BEHAVIOR | Expose exactly Wall time, Tokens, and Models with their exact semantic row-key catalogs and indices, stable row families, local period labels, measure-specific values, and friendly row and evidence labels. | `HeatmapMode`; matrix DTO union; Heatmap Visible-Range Contract; View Behavior | DEFINED | Application Service owns row construction, aggregation, formatting, and source order | Mode inventory, semantic-key, row-order, label, local-time, and value-format tests |
...
Wall time rows include only runtime states present in the snapshot. Known rows use these exact `rowKey` values in order: `model_inference`, `tool_execution`, `test_process`, `agent_wait`, `user_pause`, `watchdog`, `approval_infrastructure`, and `unattributed`. Their actor-visible labels remain Model inference, Tool execution, Build / Test, Waiting for agent, User pause, Watchdog, Approval / infrastructure, and Unattributed. Other present states follow with `runtime:<normalized-state>` keys in ascending suffix order and use title-cased labels.

Tokens rows use these exact `rowKey` values in order: `uncached_input_tokens`, `cached_input_tokens`, `reasoning_tokens`, `output_tokens`, `tool_calls`, `context_average`, `context_maximum`, and `cost`. Their actor-visible labels remain Uncached input, Cached input, Reasoning, Output, Tool calls, Context size (avg), Context size (max), and Cost. Models rows use one `model:<24-lowercase-hex-digest>` key per normalized model-and-effort identity in service-owned first-occurrence order and place `cost` last. Every row has a zero-based `rowOrderIndex` equal to its array index. The Workspace validates these semantic keys and indices without sorting, adding, merging, or relabeling rows.
...
Each non-context row has its own available scale with `basis="visible_row_maximum"`. The `context_average` and `context_maximum` rows have an available `context_window_capacity` scale only when capacity is known. An unavailable scale has no numeric domain. The parser selects these validation rules from `rowKey`, never from the friendly label. A localized or changed label cannot change scale validity. The Workspace never derives a scale or intensity from another row.
...
The parser verifies `rows.length <= maximumRows`, exact mode-specific `rowOrder`, unique semantic row keys, contiguous `rowOrderIndex` values from zero, the exact semantic catalog and order for the mode, and `totalCellCount === sum(row.cells.length)`. It rejects totals above 2,000, unsupported resolution values, mixed scale variants, scale-to-`rowKey` mismatches, non-finite numeric values, unordered or overlapping periods, identity mismatches, and fields from the wrong query variant.

Selection state retains `rowId`, `rowKey`, and `rowOrderIndex` from the accepted matrix row. A `cell_evidence` result echoes all three values. The parser accepts that result only when its snapshot, revision, mode, row identity, semantic key, order index, and half-open period equal the pending selection. `rowLabel` remains actor-visible text and is not a correlation key.

The parser applies the Worker escaped-content limits before rendering: 256 bytes for each row or evidence label; 64 bytes for each matrix or evidence formatted value; 80 bytes for nullable matrix supporting text; 4,096 bytes for nullable evidence preview; and at most 32 non-empty provenance strings of 256 bytes each. Counts use UTF-8 bytes after JSON escaping and exclude the surrounding quotation marks. The native boundary also rejects a complete compact JSONL record plus line feed above 1,048,576 bytes.

An immutable pricing assessment can affect only returned Cost cell and evidence semantics. The Workspace receives no pricing table, model-price mapping, pricing assessment object, or raw pricing input. It renders the returned raw nullable value, formatted value, state, support, and evidence method without recalculating cost.
...
| `heatmapInteraction` | Current mode, visible range, five-value period setting, selected row ID/key/index and period, bounded range history, and selected-cell evidence state | Mode or revision change clears selection and evidence. Matrix replacement keeps selection only when the exact semantic row identity and period remain valid. History holds at most 100 entries and never crosses a snapshot revision. |
...
| Heatmap modes and rows | Exactly Wall time, Tokens, and Models render. Exact semantic `rowKey` catalogs, `rowOrderIndex` values, kinds, and ordering validate without labels. Wall time uses present runtime rows in accepted order. Tokens uses the exact eight rows. Models preserves normalized model-and-effort order and Cost last. Atomic token measures are not top-level modes. |
| Heatmap matrix contract | `queryKind="matrix"`, exact half-open UTC range, mode, requested supported resolution, and row limit enter the request. Matching immutable snapshot and revision, row order, omitted count, IDs, semantic keys, indices, friendly labels, true scale unions, cells, total, and supported coarsening validate. Exactly 2,000 cells pass; 2,001 fail. Matrix responses contain no evidence list or eager detail. |
...
| Heatmap evidence | Selection sends only the exact `cell_evidence` variant. The result's echoed row ID, semantic key, and order index must match the selected matrix row. Raw nullable value, formatted value, nullable duration, friendly label, bounded sanitized preview, evidence method, value state, and detail availability render. At most 100 chronological rows and the exact omitted count pass. Full detail remains lazy. |
| Heatmap payload boundaries | Escaped strings at the 256-byte label, 64-byte formatted-value, 80-byte supporting-text, 4,096-byte preview, and 32-by-256-byte provenance boundaries pass. Quotes, reverse solidi, control escapes, and multibyte Unicode count by escaped UTF-8 content bytes. One escaped byte over each field limit fails. A complete compact JSONL record plus line feed at 1,048,576 bytes passes; 1,048,577 bytes fails. |
