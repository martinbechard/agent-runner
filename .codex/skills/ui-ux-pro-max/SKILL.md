...
The search script is bundled with this repository-local skill. From the
repository root, invoke it through the project-local skill path:

```bash
python3 .codex/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

If `python3` is unavailable, try `python`, then `py -3`. The script requires
Python 3 and only the standard library. Do not install Python or modify the
operating system on the user's behalf; report the missing runtime as a blocker.

If the current working directory is not the repository root, resolve the
absolute repository root first and invoke the same
`.codex/skills/ui-ux-pro-max/scripts/search.py` file from there. Do not use
plugin-root environment variables or assume a user-global skill installation.
...
# Step 4: stack guidelines
python3 .codex/skills/ui-ux-pro-max/scripts/search.py "suspense streaming bundle" --stack nextjs
```
...
For every UI delivery or audit, read `references/quick-reference.md` only for
the applicable priority categories and run their relevant checks. For
native/mobile UI (iOS, Android, React Native, or Flutter), also read
`references/pro-rules.md` and run its canonical Pre-Delivery Checklist for
icon discipline, touch feedback, light/dark contrast, safe areas, and mobile
accessibility. Do not apply native-only requirements to web or desktop UI.

For a scored or durable audit, complete
`references/evidence-review-checklist.md`. Record each search query and matched
rule identity used to justify a finding, and label general fallback guidance
when no verified database match exists.
