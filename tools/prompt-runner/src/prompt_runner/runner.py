...
def _emit_progress(message: str) -> None:
    ...
    sys.stdout.flush()
    aggregate = os.environ.get("PROMPT_RUNNER_PROCESS_LOG")
    if aggregate:
        _append_process_log_path(Path(aggregate), "runner", message)
...
class RunConfig:
    ...
    debug: int = 0
    """When > 0, write depth-limited function tracing to process.log."""
...
def _process_log_path(run_dir: Path) -> Path:
    return _run_files_dir(run_dir) / "process.log"


def _append_process_log_path(path: Path, source: str, message: str) -> None:
...
def _append_process_log(run_dir: Path, source: str, message: str) -> None:
    _append_process_log_path(_process_log_path(run_dir), source, message)


@contextmanager
def _debug_trace(run_dir: Path, debug_depth: int):
    if debug_depth <= 0:
        yield
        return
    ...
    _append_process_log(run_dir, "debug-trace", f"enabled depth={debug_depth}")
    try:
        yield
    finally:
        sys.setprofile(previous_profile)
        threading.setprofile(None)
        _append_process_log(run_dir, "debug-trace", "disabled")
...
def _format_included_files_section(
    ...
    if not pair.include_files:
        return ""
    blocks: list[str] = []
    for raw_path in pair.include_files:
...
def _ensure_run_files_gitignored(run_dir: Path) -> None:
    ...
    if _git_is_worktree(run_dir):
        ...
        exclude_path = (
            ...
            else (run_dir / raw_exclude_path).resolve()
        )
...
def _read_saved_prompt_artifact_text(
    *,
    ...
    """Read the real saved artifact text from the worktree when possible.

    Prompt-runner no longer maintains a duplicate final-artifact sidecar.
    Resume and judge-only rebuild their state from the actual created file.
    """
...
def _make_call(
    *,
    ...
    prompt_dir = _prompt_artifact_dir(run_dir=worktree_dir, pair=pair)
    stdout_log_path = (
        prompt_dir / f"prompt-{pair.index:02d}.iter-{iteration:02d}-{role}.stdout.log"
    )
    stderr_log_path = (
        prompt_dir / f"prompt-{pair.index:02d}.iter-{iteration:02d}-{role}.stderr.log"
    )
    ...
    return ClaudeCall(
        ...
        aggregate_log_path=_process_log_path(run_dir),
        aggregate_log_prefix=(
            f"{role} prompt={pair.index} iter={iteration}"
        ),
    )
...
def _run_interactive_prompt(
    ...
    mission = build_initial_generator_message(
        pair, prior_artifacts, worktree_dir, generator_prelude=config.generator_prelude,
    )
...
def run_prompt(
    ...
    for iteration_number in range(1, config.max_iterations + 1):
        ...
        _append_log(
            ...
            f"agent_name: {gen_call.agent_name}\n",
        )
        ...
        _append_log(
            ...
            f"agent_name: {jud_call.agent_name}\n",
        )
...
def _load_saved_prompt_state(
    ...
    artifact_text = _read_saved_prompt_artifact_text(
        pair=pair,
        worktree_dir=worktree_dir,
        created_files=files,
    )
...
def _load_prior_artifact_from_disk(
    pair: PromptPair, run_dir: Path, worktree_dir: Path,
) -> PriorArtifact:
    """Reconstruct a PriorArtifact for a completed prompt from its on-disk
    artifact file and files-created files. Used by --resume to rebuild the
...
def _resolve_deterministic_validation_script(
    ...
    if source_file is not None:
        prompt_relative_candidate = (source_file.parent / raw_path).resolve()
        if prompt_relative_candidate.exists():
            return prompt_relative_candidate
    return candidate
...
def _render_inline_includes(
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        try:
            ...
            raise FileNotFoundError(
                f'Inline include "{raw_target}" could not be read at "{resolved}": {exc}'
            ) from exc
...
def _render_prompt_pair(
    ...
    return PromptPair(
        ...
        include_files=tuple(
            _render_placeholders(path, context) for path in pair.include_files
        ),
        checks_files=tuple(
...
def _run_deterministic_validation(
    *,
    ...
    stderr_log_path = (
        prompt_dir / f"prompt-{pair.index:02d}.iter-{iteration_number:02d}-deterministic-validation.stderr.log"
    )
...
def _run_fork_point(
    ...
    if config.variant is not None:
        selected_variants = [
            variant for variant in fork.variants
            if variant.variant_name == config.variant
        ]
        if not selected_variants:
            raise ValueError(
                f"fork prompt {fork.index} '{fork.title}' has no variant named "
                f"'{config.variant}'"
            )
    ...
    for variant in selected_variants:
        ...
        child_env["PROMPT_RUNNER_PROCESS_LOG"] = str(_process_log_path(run_dir))
        proc = subprocess.Popen(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
...
def run_pipeline(
    ...
    try:
        ...
        for item_index, item in enumerate(pairs):
            ...
            rendered_pair = _render_prompt_pair(
                pair,
                placeholder_context,
                worktree_dir,
                source_file,
            ...
            if still_skipping:
                if config.judge_only == rendered_pair.index:
                    ...
                    verdict_lines = (
                        verdict_path.read_text(encoding="utf-8").splitlines()
                        if verdict_exists
                        else []
                    )
                    if verdict_lines[:1] == ["pass"]:
                        prior_artifact = _load_prior_artifact_from_disk(
                            rendered_pair,
                            run_dir,
                            worktree_dir,
                        )
                        ...
                        created_files = [
                            Path(line) for line in files_txt.splitlines()
                            if line.strip()
                        ]
                        ...
                        print(
                            f"[resume] skipping prompt {rendered_pair.index} "
                            f"'{rendered_pair.title}' — already pass",
                            flush=True,
                        )
                        _emit_progress(
                            f"{_progress_prefix(started_at)} "
            ...
            if missing_required:
                ...
                halt_reason = (
                    f"R-MISSING-REQUIRED-FILES: prompt {rendered_pair.index} "
                    f"\"{rendered_pair.title}\" is missing required files:\n"
                    f"{missing_lines}"
                )
                _emit_progress(
                    f"{_progress_prefix(started_at)} "
            ...
            if deterministic_script is not None and not deterministic_script.exists():
                halt_reason = (
                    ...
                    f"{rendered_pair.index} \"{rendered_pair.title}\" is "
                    f"missing deterministic validation script:\n"
            ...
            try:
                ...
                halt_reason = (
                    f"R-NO-VERDICT: prompt {rendered_pair.index} "
                    f"\"{rendered_pair.title}\" returned a judge response "
                    f"with no VERDICT line. {err}"
                )
                _emit_progress(
                    f"{_progress_prefix(started_at)} "
        ...
        if previous_process_log_env is None:
            os.environ.pop("PROMPT_RUNNER_PROCESS_LOG", None)
        else:
