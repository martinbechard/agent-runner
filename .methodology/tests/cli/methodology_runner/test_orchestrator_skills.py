...
def test_orchestrator_runs_selector_once_per_phase_and_writes_manifest(tmp_path: Path):
    ...
    catalog = {
        "ph000-requirements-extraction": SkillCatalogEntry(
            id="ph000-requirements-extraction", description="Extract reqs",
        ...
        "ph000-requirements-quality-review": SkillCatalogEntry(
            id="ph000-requirements-quality-review", description="QA reqs",
    ...
    client = _ScriptedClaude(responses=[
        _minimal_selector_reply(
            "PH-000-requirements-inventory",
            gen=["ph000-requirements-extraction", "traceability-discipline"],
            jud=["ph000-requirements-quality-review", "traceability-discipline"],
        ),
    ])
...
def test_orchestrator_uses_file_reference_prelude_for_codex(tmp_path: Path):
    ...
    assert str(catalog["ph000-requirements-extraction"].source_path) in gen_text
    assert "body for ph000-requirements-extraction" not in gen_text
