...
See .methodology/docs/design/components/CD-002-methodology-runner.md Section 8.
...
When iterating coverage_check, skip the reserved `status` metadata entry. It is
generated by phase_0_validation for a human-readable summary and is not a source
phrase mapping.
Only flag a coverage failure when the authoritative coverage_verdict fails, a
non-status coverage_check entry references a nonexistent RI-* item, or a source
phrase's mapped RI-* items do not actually preserve the requirement meaning.
