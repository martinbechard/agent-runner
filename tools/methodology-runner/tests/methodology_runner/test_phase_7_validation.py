...
def test_phase_7_validation_accepts_non_literal_final_verification_title(
    tmp_path: Path,
) -> None:
    feature_spec = _write(
        tmp_path / "docs" / "features" / "feature-specification.yaml",
...
## Prompt 2: Run Full Verification And Preserve The Evidence

### Generation Prompt

Run the full verification commands and preserve the final command evidence in
...
""",
    ...
    assert report["overall_status"] == "pass"
    assert "workflow_final_verification_prompt" not in report["failed_checks"]


def test_phase_7_validation_rejects_methodology_self_validation_command(
    tmp_path: Path,
) -> None:
    feature_spec = _write(
        tmp_path / "docs" / "features" / "feature-specification.yaml",
...
Use TDD and run full verification commands.
...
""",
  ...
  - command: "pytest -q"
    ...
    purpose: "Verify the application output test passes."
    evidence: "tests/test_app.py passes against app.py"
  - command: "python3 -m methodology_runner.phase_7_validation --verification-report docs/verification/verification-report.yaml"
    exit_code: 1
    purpose: "Check the methodology package itself."
...
""",
    ...
    assert report["overall_status"] == "fail"
    assert "verification_commands_workspace_scope" in report["failed_checks"]
