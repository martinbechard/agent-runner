...
class ClaudeCall:
    ...
    """Logical agent identity for this call. Codex uses this to load a
    custom agent contract; other backends may ignore it."""
    aggregate_log_path: Path | None = None
    """Optional run-scoped append-only log that receives live stdout/stderr
    with a prefixed source label."""
    aggregate_log_prefix: str = ""
    """Prefix written into aggregate_log_path for each streamed line."""
