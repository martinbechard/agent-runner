...
_PHASE_0 = PhaseConfig(
    ...
    generation_instructions=(
        ...
        "\n"
        "For large requests, first run the deterministic PH-000 bootstrap\n"
        "validator with --generate to create a source-faithful seed inventory\n"
        "and coverage file, then let judge and cross-reference checks identify\n"
        "any semantic revisions that are still needed.\n"
        "\n"
        ...
        "items.\n"
        "\n"
        "Treat bullets, numbered entries, and table rows under\n"
        "requirement-bearing lead-ins such as must support, must include,\n"
        "must show, must provide, must expose, must cover, or must handle\n"
        "as separate requirements when they describe independently\n"
        "satisfiable behaviours, capabilities, constraints, supported input\n"
        "shapes, user actions, error cases, security rules, test scenarios,\n"
        "or acceptance criteria.\n"
        "\n"
        "Do not split coherent schema, data model, table-column, metric, or\n"
        "viewer-control lists into one RI item per field name solely because\n"
        "the fields are individually listed.  Preserve the complete grouped\n"
        "list in the normalized requirement and use the coverage file to map\n"
        "the source phrase to that RI item.\n"
        "\n"
        "Use only contiguous source wording in verbatim_quote.  Do not\n"
        "concatenate non-adjacent bullets, skip intervening bullets inside\n"
        "one quote, or quote an entire list when its entries are separately\n"
        "satisfiable.  Use the lead-in sentence as context for\n"
        "normalized_requirement and source_location instead.\n"
        "\n"
...
_PHASE_1 = PhaseConfig(
    ...
    extraction_focus=(
        ...
        "'fast', 'user-friendly', 'appropriate', 'when practical',\n"
        "'does not freeze') unless the qualifier is anchored to observable\n"
        "feature-level behavior.  Local\n"
        ...
        "  Bound preservation: lower and upper bounds such as at least one,\n"
        "at most one, and one or more must not drift into unsupported exact\n"
        "counts.  Dependency identification: features that share data,\n"
        "sequencing constraints, transactional boundaries, access-control\n"
        "boundaries, viewer surfaces, parser output, or navigation behavior\n"
        "must declare their inter-feature dependencies."
    ),
    generation_instructions=(
        ...
        "qualitative performance or responsiveness requirements as observable\n"
        "product behavior when possible, such as what UI state appears before\n"
        "optional large content is opened, whether controls stay available\n"
        "while optional content loads, and whether loading, truncation,\n"
        ...
        "in that same feature's description or AC-* entries.  Preserve lower\n"
        "bounds like at least one as minimums instead of exact counts, and\n"
        "state security boundaries as allowed operations plus rejected\n"
        "operations.  Include dependencies when a feature relies on another\n"
        "feature's access-control boundary, viewer surface, parser output, or\n"
        "navigation behavior.  The\n"
    ...
    judge_guidance=(
        ...
        "   without a measurable threshold or concrete observable completion\n"
        "   condition.  Performance or responsiveness ACs without numeric\n"
        "   thresholds are acceptable only when they state observable product\n"
        "   behavior such as visible initial data, controls that remain\n"
        "   available while optional content loads, or explicit loading,\n"
        "   truncation, warning, or error states.\n"
...
_PHASE_2_ARCHITECTURE = PhaseConfig(
    ...
    generation_instructions=(
        ...
        "tests, scripts, and other deliverables. For a truly single-component\n"
        "application with no meaningful internal or external provider,\n"
        "service, library, API, dependency-injection, command, or module\n"
        "boundary, set simulation_target false, simulation_boundary none,\n"
        ...
        "A single local or deployable application can still have multiple\n"
        "architecture components when it contains real browser-to-server,\n"
        "server-to-library, adapter-to-provider, dependency-injection, API\n"
        "route, package/module, command, or service boundaries that help\n"
        "downstream phases design, simulate, build, or integration-test the\n"
        "system in stages. Only collapse to one component when there is no\n"
        "meaningful internal or external implementation boundary.\n"
        "\n"
    ...
    judge_guidance=(
        ...
        "8. Boundary overcorrection: do not reject internal code boundaries\n"
        "   solely because the requested software is one local or deployable\n"
        "   application. Browser/server, API, library, package/module,\n"
        "   dependency-injection, command, and service boundaries are valid\n"
        "   when they provide downstream design, simulation, build, or\n"
        "   integration-test value.\n"
        "9. Missing rationale: the rationale field must explain the\n"
...
_PHASE_5 = PhaseConfig(
    ...
    extraction_focus=(
        ...
        "Contract fidelity: simulations must model contract-visible success,\n"
        "error, and invalid-request outcome families, including runtime\n"
        "validation for enum-like request fields when a boundary can receive\n"
        "dynamic input.\n"
    ...
    generation_instructions=(
        ...
        "For every referenced contract operation, enumerate request fields,\n"
        "normal outcomes, error outcomes, and enum/literal values.  The fake\n"
        "must defensively validate runtime enum/literal values before lookup or\n"
        "dispatch and return the contract's declared invalid-request outcome\n"
        "when unsupported values arrive through a dynamic seam.  Do not collapse\n"
        "contract-visible failures into generic thrown errors unless the\n"
        "contract explicitly defines exceptions as the boundary behavior.\n"
    ...
    judge_guidance=(
        ...
        "   Invalid-request and structured error families must be complete enough\n"
        "   for runtime integration seams, not only compile-time types.\n"
