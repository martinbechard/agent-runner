...
- **REQUIREMENT: REQ-1** Core workflow execution
  ...
  - **REQUIREMENT:** No implicit cleanup on verdict
    - **SYNOPSIS:** The runner MUST NOT roll back prompt-created artifacts automatically when a judge returns `revise` or `escalate`.
      - **BECAUSE:** Revision and escalation both depend on preserving the actual artifact state for inspection, continuation, or upstream correction.
...
- **REQUIREMENT: REQ-3** Operational robustness
  ...
  - **REQUIREMENT:** Prompt-input traceability
    - **SYNOPSIS:** Each rendered generator and judge prompt sent to a backend MUST be persisted on disk per iteration in the runner-owned forensic area.
      - **BECAUSE:** Forensic replay and debugging require the exact prompt input, not only the resulting model output and logs.
...
- **ENTITY: ENTITY-3** `PromptModule`
  ...
  - **FIELD:** `module_slug`
    - **SYNOPSIS:** One stable module identifier for the whole file.
    - **BECAUSE:** Artifact scoping belongs to the prompt file as a whole, not to individual prompt pairs inside it.
  - **FIELD:** `prompt_pairs`
...
- **ENTITY: ENTITY-8** `IterationTrace`
  - **SYNOPSIS:** Durable forensic record for one generator/judge iteration within a prompt.
  - **FIELD:** `generator_prompt_path`
    - **SYNOPSIS:** Path to the exact rendered generator input message sent to the backend for that iteration.
    - **BECAUSE:** Investigating generator behavior requires the real prompt text after placeholder resolution, preludes, and revision feedback have been applied.
  - **FIELD:** `judge_prompt_path`
    - **SYNOPSIS:** Path to the exact rendered judge input message sent to the backend for that iteration.
    - **BECAUSE:** Judge-loop diagnosis requires the real review prompt, not only the judge's output.
  - **FIELD:** `generator_output_path`
    - **SYNOPSIS:** Path to the captured generator output artifact for that iteration.
    - **BECAUSE:** Input and output must be inspectable as a pair.
  - **FIELD:** `judge_output_path`
    - **SYNOPSIS:** Path to the captured judge output artifact for that iteration.
    - **BECAUSE:** Revision reasoning depends on the exact judge response paired to the prompt that produced it.
...
- **MODULE: MODULE-3** `src/cli/prompt_runner/runner.py`
  ...
  - **WRITES:** per-iteration rendered generator and judge prompt files under `.run-files/.../history/`
    - **BECAUSE:** Prompt input persistence is part of the runner's required forensic trace, not an optional debug convenience.
  ...
  - **WRITES:** `.run-files/<module-slug>/summary.txt`
    - **BECAUSE:** The summary belongs to the prompt file's module-scoped forensic area, alongside that module's history and logs.
